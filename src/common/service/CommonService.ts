...
export abstract class CommonService<U> extends EventEmitter2 implements BaseService<U> {
    ...
    protected constructor(vue: { proxy: Vue } | null) {
        super();
        this._vue = <Vue>(<{proxy: Vue}>vue).proxy;
    ...
    public hasService<T extends CommonService<any>>(clazz: new (vue: { proxy: Vue } | null) => T, index?: number): boolean {
        let serviceName = this.getServiceName((<Record<string, any>>clazz).class);
        let vue = this.getVue(serviceName, index || 0);
        return typeof vue !== "undefined";
    }

    public getService<T extends CommonService<any>>(clazz: new (vue: { proxy: Vue } | null) => T, index?: number): T {
        let serviceName = this.getServiceName((<Record<string, any>>clazz).class);
        let vue = this.getVue(serviceName, index || 0);
